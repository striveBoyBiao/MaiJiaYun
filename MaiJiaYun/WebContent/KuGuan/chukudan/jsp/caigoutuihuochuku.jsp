...
<script type="text/javascript"
	src="${pageContext.request.contextPath}/KuGuan/chukudan/js/caigou.js "></script>
<script type="text/javascript">	
...
<script type="text/javascript">
	...
	/* 	 alert(ben);
		...
		location.href="${pageContext.request.contextPath }/ConfirmChukuServlet?type=caigou&ben="+ben+"&xu="+xu+"&yi="+yi+"&chuid="+chuid+"&beizhu="+beizhu;
	}
</script>
<script type="text/javascript">
	function bohui(id){
		if(confirm("您确认驳回吗？")){
		location.href="${pageContext.request.contextPath }/BoHuiServlet?type=caigou&id="+id;
		}
	}
</script>

<script type="text/javascript">
	var pageNum = ${cgall.pageNum};//当前页码
	var maxPage = ${cgall.maxPageNum};//最大页码
	var cangkuming = "";
	var pageSize = ${cgall.pageSize};//页面大小
	var i=1;
	
	//封装的回调函数
	function get(sousuo,pageSize,cangkuming,pageNum){
		 $.get(
 			"${pageContext.request.contextPath }/getNum?op=caigou&sousuo="+sousuo+"&pageSize="+pageSize+"&cangku="+cangkuming+"&pageNum="+pageNum,
 			function(data){
		...
		<div class="content" id="content">
			...
			<div class="collapse navbar-collapse navbar-default navbar-titletab"
				...
				<ul class="nav nav-tabs f14 fb">
					<li id="li-outpurchase" class="ml20 tab active">
						<a class="" href="${pageContext.request.contextPath }/servlet/kuguan/chukudan/one?op=caigou">采购退货出库
						<span>(${cgall.total })</span></a></li>
						<!--  ${fn:length(chukudan) }这个方法可以获得一个集合的数量-->

					<li id="li-outAllocation" class="tab">
					<a class="c6" href="${pageContext.request.contextPath }/servlet/kuguan/chukudan/one?op=diaobo">调拨出库
						<span> (${cgall.total1 })</span></a></li>
				...
				<div id="panel" class="prl10auto">
					...
					<div class="panel mt15">
						...
						<table class="table table-striped table-hover">
							...
							<thead class=" table-bordered">
								<tr>
									...
									<ul id="cangkuanniu" aria-labelledby="dropdownMenu1" role="menu" class="dropdown-menu">
										<c:forEach items="${cgall.cangkus}" var="ccku" varStatus="status">
											<li role="presentation">
												<a href="#" tabindex="-1" onclick="xzck(${status.index})" role="menuitem" warehouseid="18394" type="自有仓库">${ccku} </a></li>
										</c:forEach></ul>
										</div>
									</th>
									<!--  仓库按钮结束-->
							...
							<tbody id="orderList">
								<% int i = 1;%>
								<c:forEach items="${cgall.data}" var="cga" varStatus="status">
									<tr class="order" modelid="3422962">
										<!-- 前面的多选框开始 -->
										<td style="background-color: rgb(240, 248, 253);">
										<input id=<%=i++%> name="checkOrder" value="${cga.chukudanId}" style="margin-right: 5px;" type="checkbox"> 
											<% int y = i - 1; %> </td>
										<!-- 前面的多选框结束 -->
										
										<td style="background-color: rgb(240, 248, 253);"><a onclick="changec(<%=y%>)">${cga.chukudanCode }</a></td>
										<td style="background-color: rgb(240, 248, 253);">${cga.chukuxiangguandanCode }</td>
										<td style="background-color: rgb(240, 248, 253);">${cga.chukuCangku }</td>
										<td style="background-color: rgb(240, 248, 253);">${cga.chukuNum }</td>
										<td style="background-color: rgb(240, 248, 253);">${cga.chukudanTime }</td>
										<td style="background-color: rgb(240, 248, 253);">${cga.chukuPeople }</td>
										<td style="background-color: rgb(240, 248, 253);" colspan="2">
										<a onclick=" showdetail(${cga.chukudanId})">
											...
											<button class="btn btn-default operate-reject" type="button" onclick="bohui(${cga.chukudanId})">驳回到采购退货单</button>
										</td>
									</tr>
								</c:forEach>
							</tbody>
				...
				<div class="modal-content">
					...
					<div class="modal-body" style="padding-top: 0px;">
						<form class="form-inline" role="search" id="form1" method="post">
							<input name="warehouseVoucherId" value="3422962" type="hidden">
							<!--part1 star-->
							<div class="btn-group-block">
								<!--search star-->
								...
								<div class="form-group mr20">
									<div class="input-group"><input id="goodsSku" class="form-control" type="text"></div>
								</div>
			...
			<div style="float: right;">
					...
					<ul class="pagination">
						<li class="dropup">
							...
							<div class="btn-group" style="margin: 0 5px;">
								<span class="lh30 left fb red" id="totalRecords">${cgall.maxNum }</span>
				...
				<div style="width: 350px; float: left;" id="pagingPages">
					<ul class="pagination">
						...
						<c:forEach var="x" begin="1" end="${cgall.maxPageNum}" step="1"> 
	         				<c:if test="${x < 5}" >
